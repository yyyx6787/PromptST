...
from model.MTGNN import MTGNN as Network
# from model.MTGNN import PGNN as Network1
# from model.MTGNN import STMLP as Network2
from model.MTGNN import MulP as Network3
...
args.add_argument('--teacher', default=False, type=eval)
...
# #prompt model
# pmodel = Network1(args)
# pmodel = pmodel.to(args.device)

# #prompt model
# pmodel = Network2(args)
# pmodel = pmodel.to(args.device)

#prompt model
pmodel = Network3(args)
pmodel = pmodel.to(args.device)
...
# print_model_parameters(model, only_num=False)  #printing model parameters

#load dataset
train_loader, val_loader, test_loader, prompt_loader, scaler_data, scaler_day, scaler_week = get_dataloader(args,
...
optimizerp = torch.optim.Adam(params=pmodel.parameters(), lr=args.lr_init, eps=1.0e-8,
							 weight_decay=0, amsgrad=False)
...
if args.lr_decay:
	...
	lr_schedulerp = torch.optim.lr_scheduler.MultiStepLR(optimizer=optimizerp,
														milestones=lr_decay_steps,
														gamma=args.lr_decay_rate)
	# lr_schedulerp = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizerp, T_max=64)

#config log path
current_dir = os.path.dirname(os.path.realpath(__file__))
log_dir = os.path.join(current_dir,'SAVE1', args.dataset)
Mkdir(log_dir)
args.log_dir = log_dir

#start training
trainer = Trainer(model, pmodel, loss, optimizer, optimizerp, train_loader, val_loader, test_loader, prompt_loader, scaler_data,
				  args, lr_scheduler=lr_scheduler, lr_schedulerp = lr_schedulerp)
if args.mode == 'train':
	trainer.train()
elif args.mode == 'test':
	model.load_state_dict(torch.load(log_dir + '/best_model.pth'))
	pmodel.load_state_dict(torch.load(log_dir + '/best_model_prompt.pth'))
	print("Load saved model")
	trainer.test(model, pmodel, trainer.args, test_loader, scaler_data, trainer.logger)
elif args.mode == 'fine':
	model.load_state_dict(torch.load(log_dir + '/best_model.pth'))
	print("Load saved model")
	trainer.trainF(model)
elif args.mode == 'prompt':
	model.load_state_dict(torch.load(log_dir + '/best_model.pth'))
	print("Load saved model")
	trainer.trainP(model)
	# trainer.prompt(model, trainer.args, test_loader, prompt_loader, scaler_data, trainer.logger)
else:
	raise ValueError
